...
export const migration_1_0_0_to_1_1_0: Migration = {
	...
	migrate: (data: unknown) => {
		...
		console.info(
			`[Migration 1.0.0 → 1.1.0] Found ${challengeKeys.length} challenge states to process`
		);
		...
		for (const key of challengeKeys) {
			try {
				...
				console.info(
					`[Migration 1.0.0 → 1.1.0] Processing ${key}: challengeId=${actualData.challengeId}, seed=${actualData.seed}, currentBossOrder=${actualData.currentBossOrder}`
				);
				...
				console.info(
					`[Migration 1.0.0 → 1.1.0] ✓ Marked bosses 1-${actualData.currentBossOrder} as revealed for ${actualData.challengeId} (seed: ${actualData.seed})`
				);
			} catch (error) {
