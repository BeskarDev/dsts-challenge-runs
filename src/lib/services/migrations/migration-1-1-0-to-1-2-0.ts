...
export const migration_1_1_0_to_1_2_0: Migration = {
	...
	toVersion: '1.2.0',
	description:
		'Update to digivolution naming, checkpoint structure, and boss group animation logic',
	migrate: (data: unknown) => {
		...
		console.info(
			`[Migration 1.1.0 → 1.2.0] Found ${challengeKeys.length} challenge states to update animation state`
		);
		...
		for (const key of challengeKeys) {
			try {
				...
				const actualData =
					challengeState && typeof challengeState === 'object' && 'data' in challengeState
						? challengeState.data
						: challengeState;
				...
				if (
					!actualData.challengeId ||
					!actualData.seed ||
					typeof actualData.currentBossOrder !== 'number'
				) {
				...
				if (bossGroups.length === 0) {
					console.info(
						`[Migration 1.1.0 → 1.2.0] No boss groups for ${actualData.challengeId}, skipping animation update`
					);
					continue;
				}
				...
				console.info(
					`[Migration 1.1.0 → 1.2.0] Processing ${key}: challengeId=${actualData.challengeId}, currentBossOrder=${actualData.currentBossOrder}, rerollTeamPerBoss=${rerollTeamPerBoss}`
				);

				if (rerollTeamPerBoss) {
					// rerollTeamPerBoss is enabled, animation state is already per-boss from 1.1.0 migration
					console.info(
						`[Migration 1.1.0 → 1.2.0] ✓ Run has rerollTeamPerBoss=true, keeping per-boss animation state`
					);
				} else {
					...
					for (const startBoss of groupsToMark) {
						...
						if (group) {
							...
							console.info(
								`[Migration 1.1.0 → 1.2.0] ✓ Marked boss group ${group.startBoss}-${endBoss} (${group.label})`
							);
						}
					}
					console.info(
						`[Migration 1.1.0 → 1.2.0] ✓ Marked ${groupsToMark.size} boss groups as revealed for ${actualData.challengeId}`
					);
				}
			} catch (error) {
