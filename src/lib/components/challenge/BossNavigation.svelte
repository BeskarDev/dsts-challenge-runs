<script lang="ts">
	...
	const previousBoss = $derived(
		bosses.filter((b) => b.order < currentBossOrder).sort((a, b) => b.order - a.order)[0]
	);
...
<div
	class="rounded-md border border-border-light dark:border-border bg-white dark:bg-[rgba(8,14,32,0.80)] backdrop-blur-sm p-4 shadow-panel-light dark:shadow-panel"
>
	<h3 class="text-lg font-semibold text-gray-900 dark:text-muted-50 mb-4">Boss Progress</h3>

	<div class="flex items-center justify-between gap-4">
		<Button variant="outline" onclick={goBack} disabled={!canGoBack} class="flex items-center gap-1">
			<svg
		...
		<div class="flex-1 text-center">
			{#if currentBoss}
				<div class="font-semibold text-gray-900 dark:text-muted-50">
					{currentBoss.name}
				...
				<div class="text-xs text-gray-500 dark:text-muted-600 mt-1">
					Boss {currentBossOrder} / {requiredBossCount}
				</div>
			{:else}
				<div class="text-gray-500 dark:text-muted">No boss selected</div>
			{/if}
		</div>
	...
	{:else}
		<div class="mt-4 pt-4 border-t border-gray-200 dark:border-border">
			<div class="text-sm text-secondary-500 dark:text-secondary-400 font-medium">
